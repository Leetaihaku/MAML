...
class PPO(BaseAgent):
    ...
    def __init__(
        ...
        self.network = Network(network_name, state_size, action_size, D_hidden=hidden_size, head=head)
        self.optimizer = Optimizer(optim_name, params=self.network.parameters())
        self.gamma = gamma
        ...
        # set weight of network to meta's
        self.network.load_state_dict(meta_weight)
    ...
    def learn(self, meta_training=False):
    ...
    def process(self, transitions, step, meta_training):
        result = None
        ...
        # Process per epi
        # TODO: Have to fix sample length(present n_step+1 -> future n_step)
        if self.learn_stamp >= self.n_step:
            result = self.learn(meta_training)
            if self.lr_decay:  # <- not use
